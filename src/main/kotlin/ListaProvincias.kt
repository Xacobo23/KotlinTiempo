...
class ListaProvincias  {
    ...
    fun getListaProvincias(): List<Provincia>{
        val jsonString=
            """
            {
              "origen": {
        ...
        val jsonURL = URI("https://www.el-tiempo.net/api/json/v2/provincias").toURL().readText()

        var gson = Gson()

        val jsonObject = JsonParser.parseString(jsonString).asJsonObject
        val provinciaArray = jsonObject.getAsJsonArray("provincias")
        val provincias = gson.fromJson(provinciaArray, Array<Provincia>::class.java).toList()
